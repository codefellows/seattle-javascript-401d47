...
const quicksort = module.exports = function (items, compare, left, right){
   ...
   // by default compare will sort things smallest to largest
...
function swap(items, left, right){
  ...
  //console.log('swapping', items[left], items[right]);
  let tmp = items[left];
...
function partition(items, compare, left, right, pivot){
  ...
  // created a pivot. piv0t will be the index of the middle of the 
  // current partition
  pivot = pivot || Math.floor((left + right) / 2);

  // continue swapping around the pivot until all items on 
  ...
  if (left <= right){ // go until the right side 
    ...
    if (left <= right){
      ...
      // but don't swap an item with its self (aka leftIndex == rightIndex)
      if (left < right) 
